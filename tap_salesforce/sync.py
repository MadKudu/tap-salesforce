...
from tap_salesforce.salesforce.exceptions import TapSalesforceQuotaExceededError
...
def sync_stream(sf, catalog_entry, state, state_msg_threshold):
    ...
    with metrics.record_counter(stream) as counter:
        try:
            ...
            singer.write_state(state)
        except TapSalesforceQuotaExceededError as ex:
            LOGGER.error(f"API limit reached for stream {stream}: {ex}. Writing state and terminating gracefully.")
            singer.write_state(state)
            return
