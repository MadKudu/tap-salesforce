...
def is_property_selected(  # noqa: C901
    stream_name,
    metadata_map,
    breadcrumb,
) -> bool:
...
def pop_deselected_schema(schema, stream_name, breadcrumb, metadata_map):
...
def do_sync(sf, catalog, state):
    ...
    try:
        ...
        sync_tasks = (sync_catalog_entry(sf, catalog_entry, state) for catalog_entry in streams_to_sync)
